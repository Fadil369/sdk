...
export class ComplianceValidator {
    ...
    initializeDefaultRules() {
        const defaultRules = [
            ...
            },
            {
                ...
                required: true,
                validate: async (context) => {
                    if (context.operation?.type === 'export' &&
            ...
            },
            {
                ...
                validate: async (context) => {
                    if (context.operation && context.user) {
                        ...
                        const hasPermission = context.user.permissions.some(p => p === requiredPermission || p === `${context.operation.resource}:*` || p === '*');
                        if (!hasPermission) {
                            return {
                                passed: false,
    ...
    async quickValidation(context) {
        const criticalRules = Array.from(this.rules.values()).filter(rule => rule.severity === 'critical' && rule.required);
        let criticalFailures = 0;
        const failedRules = [];
 ...
 */
export function createComplianceValidator(logger) {
