...
import { EncryptionKey } from '@/types/security';
...
export class EncryptionService {
   ...
   */
  async generateRSAKeyPair(keyId?: string): Promise<{ publicKeyId: string; privateKeyId: string }> {
    ...
    const publicEncryptionKey: EncryptionKey = {
      id: publicKeyId,
    ...
    const privateEncryptionKey: EncryptionKey = {
      id: privateKeyId,
      algorithm: this.config.rsa.algorithm,
      key: privateKey,
      createdAt: new Date().toISOString(),
   ...
   */
  async decryptWithAES(encryptedData: EncryptedData): Promise<string> {
   ...
   */
  async decryptWithRSA(encryptedData: EncryptedData): Promise<string> {
   ...
   */
  async rotateKeys(): Promise<{ rotated: string[]; failed: string[] }> {
    ...
    for (const [keyId, key] of this.keys.entries()) {
      try {
        ...
        failed.push(keyId);
        this.logger.error('Failed to rotate key', error as Error, { keyId });
      }
    }
